...
class ConsoleApp:
    ...
    def patron_search(self) -> ConsoleState:
        search_input = input(
            "Enter a string to search for patrons by name: ").strip()
        if not search_input:
            print("No input provided. Please try again.")
            return ConsoleState.PATRON_SEARCH
        self.matching_patrons = self._patron_repository.search_patrons(
            search_input)
    ...
    def _print_loans(self, loans):
        ...
        for loan in loans:
            if not getattr(loan, 'book_item', None) or not getattr(loan.book_item, 'book', None):
                ...
                returned = "True" if getattr(
                    loan, 'return_date', None) else "False"
                print(
                    f"{idx}) {loan.book_item.book.title} - Due: {loan.due_date} - Returned: {returned}")
    ...
    def _handle_patron_details_selection(self, selection, patron, valid_loans):
        if selection == 'q':
            ...
            print(status)
            self.selected_patron_details = self._patron_repository.get_patron(
                patron.id)
            return ConsoleState.PATRON_DETAILS
        elif selection.isdigit():
            idx = int(selection)
    ...
    def loan_details(self) -> ConsoleState:
        ...
        if selection == 'q':
            ...
            print(status)
            self.selected_loan_details = self._loan_repository.get_loan(
                loan.id)
            return ConsoleState.LOAN_DETAILS
